...
def save_created_files(fake_images, file_names, save_folder="results"):
    save_root_folder = os.path.join(
        save_folder, "fake", datetime.now().strftime("%Y-%m-%d-%H%M%S")
    )
    ...
    for idx, _ in enumerate(fake_images):
        ...
        cv2.imwrite(
            os.path.join(save_root_folder, "{}.png".format(file_name)),
...
def main(argv):
    ...
    source_images, file_names = imload.make(folder, img_size=resize, gray=gray)
